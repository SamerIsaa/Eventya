        ...
        <div class="container sign-up-container">
            <div class="row">
                ...
                <div class="col-md-8 col-xs-12 sign-up-section">
                    <h2>    {{ trans('translation.signinSupplier') }}</h2>
                    @if($errors->any())
                        @component('alerts.danger')
                            {{ $errors->first() }}
                        @endcomponent
                    @endif
                    <form action="{{ route('register.supplier') }}" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            <label for="exampleInputEmail1">{{ trans('translation.city') }}</label>
                            <select class="form-control" name="city">
                                <option value="0">{{ trans('translation.chooseCity') }}</option>
                                @if($cities->count())
                        ...
                        <div class="sign-in-option">
                            <div class="forget-password">
                                <a href="{{ route('user.signin') }}">{{ trans('translation.signinPayerOption') }}</a>
                            </div>
                        </div>
